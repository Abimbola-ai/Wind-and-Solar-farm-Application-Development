...
if os.path.isfile('weather_forecast.csv'):
    oldFrame = pd.read_csv('weather_forecast.csv', header=None)
    df_diff = pd.concat([oldFrame, df], ignore_index=True). drop_duplicates()
    df_diff.to_csv('weather_fore.csv', header=False, index=False)
else:
    weather.to_csv('weather_forecast.csv')
